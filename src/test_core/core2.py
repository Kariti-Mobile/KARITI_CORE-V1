...
#Dada a imagem original, o metodo retorna uma imagem limpa e sem ruidos
def teste6(img_path):
    img = Image.open(img_path)
    enhancer = ImageEnhance.Brightness(img)
    image = enhancer.enhance(1.5)  # Aumenta o brilho
...
def _decode_image(
    ...
    def qtd_corrigidas(correcoes):
        import ast
        try:
            correcoes = ast.literal_eval("(" + correcoes + ")")
        except:
            return 0
        #print(correcoes)
        aux = {}
        qtd = 0
        for x, y in correcoes:
            if y != 0:
                aux[x] = y
        return len(aux)
    ...
    # 2
    img_atual = img_orig.copy()
    tmp = _correct(img_atual, id_prova, id_aluno, num_questions_DB, num_alternatives_DB, '2', img_path)
    if tmp.split(";")[3] == "0" and (maximo is None or qtd_corrigidas(tmp.split(";")[4]) > maximo):
        maximo = qtd_corrigidas(tmp.split(";")[4])
        saida = tmp
    # 3
    ...
    # 6 
    img_atual = teste6(img_path)
    tmp = _correct(img_atual, id_prova, id_aluno, num_questions_DB, num_alternatives_DB, '6', img_path)
    if tmp.split(";")[3] == "0" and (maximo is None or qtd_corrigidas(tmp.split(";")[4]) > maximo):
        maximo = qtd_corrigidas(tmp.split(";")[4])
        saida = tmp
    ...
    img_atual = teste3(img_atual)
    img_warp_tc = img_atual
    tmp = _correct(img_atual, id_prova, id_aluno, num_questions_DB, num_alternatives_DB, '7', img_path)
    if tmp.split(";")[3] == "0" and (maximo is None or qtd_corrigidas(tmp.split(";")[4]) > maximo):
        maximo = qtd_corrigidas(tmp.split(";")[4])
        saida = tmp

    # 8 img_atual = teste4(img_warp_tc)
    tmp = _correct(img_atual, id_prova, id_aluno, num_questions_DB, num_alternatives_DB, '8', img_path)
    if tmp.split(";")[3] == "0" and (maximo is None or qtd_corrigidas(tmp.split(";")[4]) > maximo):
        maximo = qtd_corrigidas(tmp.split(";")[4])
        saida = tmp
    ...
    # 9
    img_atual = teste5(img_warp_tc) 
    tmp = _correct(img_atual, id_prova, id_aluno, num_questions_DB, num_alternatives_DB, '9', img_path)
    if tmp.split(";")[3] == "0" and (maximo is None or qtd_corrigidas(tmp.split(";")[4]) > maximo):
        maximo = qtd_corrigidas(tmp.split(";")[4])
        saida = tmp
...
def _correct(
    ...
    try:
        ...
        if a is None:
            ...
            return ";".join(output)
        tracker += 16
        question_squares, alternative_squares, _ = getOurSqr(a, num_questions_DB, num_alternatives_DB, version)
        ...
        if (len(question_squares) != num_questions_DB) and\
           (len(alternative_squares) != num_alternatives_DB
            ...
            return ";".join(output)
        tracker += 32
        respostas_aluno = getAnswers(a, num_questions_DB, num_alternatives_DB, version)
        tracker += 64
        output.append("0")
...
if len(sys.argv) < 2:
    ...
    ######## APAGAR
    #r = decode_image("1_4_10_5.png")
    r = decode_zip("prova10-15-20-s21.zip")
    for x in r:
        print("[KARITI]", x+"-")
