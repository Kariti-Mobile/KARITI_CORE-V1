...
def getImgNoColor(img):
    ...
    *A versão em P&B dessa imagem
    ...
    #cv2.imshow("im_gray", cv2.resize(im_gray, (800,600)))
    #cv2.waitKey(0)
   
    _, im_bw = cv2.threshold(im_gray, 0, 255, cv2.THRESH_OTSU | cv2.THRESH_BINARY)
    #cv2.imshow("im_bw", cv2.resize(im_bw, (800,600)))
    #cv2.waitKey(0)
...
def getPTCrop(img):
    ...
    points_list = getInfoTriang(img)[0] #encontra os triângulos
...
def getOurSqr(im_color, num_questions_DB = None, num_alternatives_DB = None, version = None):
    ...
    #im_crop = im_color[top[1]:btt[1], top[0]:btt[0]] #corta a imagen
    im_crop = im_color.copy()
    ...
    #ContourFrame = im_crop.copy()
    #ContourFrame = cv2.drawContours(ContourFrame, contours, -1, (255, 0, 0), 4) #Desenha o contorno de interesse
    
    
    #cv2.imwrite("img-teste.jpg", ContourFrame)
    #if version == "4":
        #cv2.imshow("im_bw", cv2.resize(im_bw, (800,600)))
        #cv2.waitKey(0)
    ...
    # serão considerados aqui os contonos presentes dentro do espaço de 9% a esquerda e 6% da parte superior da imagem
    ...
    for cnt in contours:
        ...
        if moments["m00"] != 0:
            ...
            continue
        if x < width * 0.09 or y < height * 0.06:
            approx = cv2.approxPolyDP(cnt, 0.04 * cv2.arcLength(cnt, True), True)
            sqr.append(approx) # 

    squares = formatShape(sqr) # resultara em uma lista que para cada squares [[cordenadas dos vertices]]
    ...
    #--------------------- Exclui os contornos dentro do limite--------------------------------------------------------------

    threshold = 0.02 #limite
    ...
    #--------------------- Seleciona os quadrados das questões com base no eixo X--------------------------------------------------------------
    ...
    #--------------------- Seleciona os quadrados das Alternativas com base no eixo Y--------------------------------------------------------------
...
def getAnswers(img, num_questions_DB = None, num_alternatives_DB = None, version = None):
    def getProportion(a, b):
        return abs(a-b)/max(a,b)
    
    def getProportion2(a, b, threshold_teste):
        min = a - (threshold_teste/2)
        maxi = a + (threshold_teste/2)
        if b > min and b < maxi:
            return True
        else:
            return False
    ...
    #imgCop = img_croped.copy()

    #if(version == "2"):
        #for a in range(len(n)):
            #print("quest"+str(a+1), str(n[a]))

    n.sort(key=lambda x: x[1][1])   
    alt.sort(key=lambda x: x[1][0]) 
    x0 = max(n[0][0][0][0],n[0][0][1][0],n[0][0][2][0],n[0][0][3][0]) #Calcula o maior valor de X entre os quatro vértices do primeiro elemento de n
    y0 = max(alt[0][0][0][1],alt[0][0][1][1],alt[0][0][2][1],alt[0][0][3][1]) #Calcula o maior valor de Y entre os quatro vértices do primeiro elemento de alt.
    x1 = max(alt[len(alt)-1][0][0][0],alt[len(alt)-1][0][1][0],alt[len(alt)-1][0][2][0],alt[len(alt)-1][0][3][0]) #Calcula o maior valor de X entre os quatro vértices do último elemento de alt.
    y1 = max(n[len(n)-1][0][0][1],n[len(n)-1][0][1][1],n[len(n)-1][0][2][1],n[len(n)-1][0][3][1]) #Calcula o maior valor de Y entre os quatro vértices do último elemento de n.
    '''
    print("INICIO----------------------------------------------------------------------")
    print("n: "+str(version), str(len(n)))
    for a in range(len(n)):
        print("quest"+str(a+1), str(n[a]))
    print("alt: "+str(version), str(len(alt)))
    for a in range(len(n)):
        print("alt"+str(a+1), str(alt[a]))
    print("FIM----------------------------------------------------------------------")
    '''
    im_gray, im_bw = getImgNoColor(img) #Transforma a imagem para P&B
    ...
    for cnt in contours:
        ...
        if len(approx) > 4:
            ...
            #cv2.drawContours(imgCop, [approx], -1, (0, 255, 0), 2)

    #cv2.imshow("contours", cv2.resize(imgCop, (800,600)))
    #cv2.waitKey(0)
    ...
    if len(circles) > 0:
        ...
        #Elimina os circulos deferentes dos de interesse
        for i in range(len(circles)-1, -1, -1): 
            if i == len(circles)-1:
        ...
        threshold = 0.055
        threshold_teste = height * 0.024
        answ = []  # Lista para armazenar quadrados selecionados
        box = []  # Lista para armazenar círculos associados a esses quadrados.

        for i in range(len(n)):
            answ_rows = []
            box_rows = []
            for j in range(len(circles)):
                if getProportion2(n[i][1][1], circles[j][0][1], threshold_teste): 
                    answ_rows.append(n[i].copy()) # Adiciona o quadrado atual na lista
                    box_rows.append(circles[j][0].copy()) # Adiciona o centro de massa do circulo atual na lista
                
            if len(answ_rows) == 0:
                answ.append(n[i].copy())
                box.append([0,0])
            else:
                for a in range(len(answ_rows)):
                    answ.append(answ_rows[a].copy())
                    box.append(box_rows[a].copy())
          
        for i in range(len(answ)):
            answ[i].append(box[i].copy()) # aqui o answ fica com 3 posições. Na posicao [0]: 4 vertices dos quadrados das questoes; [1]: x e y central dos quadrados; [2]: x e y do circulo.

        #if(version == "4"):
            #for a in range(len(answ)):
                #print("quest"+str(a+1), str(answ[a]))
    
        letters = [] #lista que reprenta as respostas em formato de numeros
        for i in range(8):
            letters.append(i+1)
        counter = 1 # Numero representativo da questão
        gabarito_aluno = []
        for i in range(len(answ)):
            #Verifica se a resposta corresponde a alguma questão
            if answ[i][2][0] != 0: #Entra aqui apenas se a questão atual tiver alguma marcação
                for j in range(len(alt)):
                    #Verifica se resposta i corresponde à alternativa j
                    if getProportion(alt[j][1][0], answ[i][2][0]) < threshold:
                        if i > 0:
                            if answ[i][1][1] != answ[i-1][1][1]: #Se questão atual diferente da questão anterior, baseado no eixo y
            ...
            else: #Cai aqui, caso para a questão atual nao tenha marcação
                if i != 0:
                    if counter == gabarito_aluno[i-1][0]:
                        counter += 1
