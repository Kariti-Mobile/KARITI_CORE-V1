...
def desenhaCabecalho(img, largura, altura, aluno, prof, prova, turma, data, nota_prova):
    ...
    for i in range(len(cabecalho_info)):
        if c < len(cabecalho_info):
            draw.text((80, 60 + espaco),  cabecalho_info[i], font = font, fill = (0, 0, 0, 0))
            espaco += 30
        c += 1
    ...
    draw.text((80,275),  f'Nome do Aluno', font = ImageFont.truetype(fm.findfont(fm.FontProperties(family='DejaVu Sans')),16), fill = (0, 0, 0, 0)) #Ubuntu
    draw.text((80, 60 + espaco),  cabecalho_info[i], font = font, fill = (0, 0, 0, 0))
